...
def pairwise_distance(point_cloud):
  """Compute pairwise distance of a point cloud. compute Euclidean distance through matrix
    distance^2 = A^2 -2AA^T + A^T^2
  Args:
  ...
  point_cloud_inner = -2*point_cloud_inner #-2AA^T
...
def get_attention_edge_feature_layer(edge_feature, output_dim, activation, is_training=None, bn_decay=None, layer='', is_dist=False):
    edge_feature = conv2d(edge_feature, output_dim, [1,1],
                          padding='VALID', stride=[1,1],
                          bn=True, is_training=is_training,
                          scope=layer+'edge_feature', bn_decay=bn_decay)
  
    net = tf.reduce_max(edge_feature, axis=-2, keep_dims=True)

    edge_attention = conv2d(edge_feature, 1, [1,1],
                          padding='VALID', stride=[1,1],
                          bn=True, is_training=is_training,
                          scope=layer+'edge_attention', bn_decay=bn_decay)

    edge_attention = tf.transpose(edge_attention, [0, 1, 3, 2])

    coefs = tf.nn.softmax(tf.nn.leaky_relu(edge_attention))
